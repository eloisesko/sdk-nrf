 ...
 * This is the device we are streaming audio to/from.
 */
static struct bt_conn *gateway_conn_peer;

int ble_acl_gateway_service_discover(struct bt_conn *conn/*, uint8_t channel_num*/)
{
#if (CONFIG_BT_VCS_CLIENT)
	int ret;

	ret = ble_vcs_discover(conn, 0);
	ERR_CHK_MSG(ret, "VCS discover failed");
#endif /* (CONFIG_BT_VCS_CLIENT) */
	return 0;
}
...
{
	LOG_DBG("Connected - nRF5340 Audio gateway");
	gateway_conn_peer = bt_conn_ref(conn);
	ble_acl_common_conn_peer_set(gateway_conn_peer);
}
